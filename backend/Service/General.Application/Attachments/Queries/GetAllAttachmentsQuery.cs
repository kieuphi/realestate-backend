...
{
    public class GetAllAttachmentsQuery : IRequest<List<Models.AttachmentModel>>
    {
        public string Type { get; set; }
    }

    public class GetAllAttachmentsQueryHandler : IRequestHandler<GetAllAttachmentsQuery, List<Models.AttachmentModel>>
    {
        private readonly IMapper _mapper;
        private readonly IApplicationDbContext _context;

        public GetAllAttachmentsQueryHandler(IApplicationDbContext context, IMapper mapper)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));
            _mapper = mapper ?? throw new ArgumentNullException(nameof(mapper));
        }

        public async Task<List<Models.AttachmentModel>> Handle(GetAllAttachmentsQuery request, CancellationToken cancellationToken)
        {
