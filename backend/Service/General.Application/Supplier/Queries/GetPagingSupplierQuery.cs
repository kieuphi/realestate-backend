...
{
    public class GetPagingSupplierQuery : IRequest<PaginatedList<ProfileInformationModel>>
    {
        public PagingIndexModel PagingModel { set; get; }
    }

    public class GetPagingSupplierQueryHandler : IRequestHandler<GetPagingSupplierQuery, PaginatedList<ProfileInformationModel>>
    {
        ...
        public GetPagingSupplierQueryHandler(
            IMapper mapper,
        ...
        public async Task<PaginatedList<ProfileInformationModel>> Handle(GetPagingSupplierQuery request, CancellationToken cancellationToken)
        {
            var model = request.PagingModel;
            string host = _commonFunctionService.ConvertImageUrl("");
            var users = (await _identityService.GetUsersInRoleAsync(Roles.Guest)).ToList();
            ...
            if (users != null && users.Count() > 0)
            {
                ...
                for (int i = 0; i < users.Count(); i++)
                {
                    ...
                    if (profile != null)
                    {
                        ...
                        profile.PropertyCount = properties.Where(x => x.SupplierId == profile.UserId).ToList().Count();
